import { Meta, Story, Props, ArgsTable, Canvas, Preview } from '@storybook/addon-docs';
import Spacer from '../../components/Spacer.vue';

<Meta
	title="Componentes/Utils/Spacer"
	component={ Spacer }
	argTypes={{
		paddingTop: {
			control: {
				type: 'range',
				min: 0,
				max: 18,
				step: 1,
			},
		},
		paddingRight: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		paddingBottom: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		paddingLeft: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		marginTop: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		marginRight: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		marginBottom: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
		marginLeft: {
			control: {
				type: 'range',
				min: 0,
				max: 12,
				step: 1,
			},
		},
	}}
	parameters={{
		viewMode: 'docs',
		previewTabs: { canvas: { hidden: true }},
		docs: {
			source: {
				language: 'html',
				format:true,
				code: /*html*/ 
`<cds-truncate
	width="80"
>
	Texto longo para testar o truncate
</cds-truncate>`
			},
		}
	}}
/>

export const Template = (args) => ({
	components: { CdsSpacer: Spacer },
	setup() {
		return { args };
	},
	template: /*html*/ `
		<div style="width: fit-content; background-color: #def7eb; border-radius: 20px">
			<div
				style="padding: 0.5px; background-color: #def7eb; border-radius: 20px"
			>
				<cds-spacer
					v-bind="args"
				>
					<div style="width: 100px; height: 100px; background-color: #2AC092; border-radius: 20px"/>
				</cds-spacer>
			</div>
		</div>
	`,
	methods: {
		logClickableClick(event) {
			console.info('logClick: ', event);
		}
	},
});

# Spacer

### Componente auxiliar utilizado para adicionar paggins e margins seguindo o padrão de tokens do Cuida.
---
<br />

## Quando usar:
- No caso de determinado componente ou elemento da interface precisar apenas de espaçamento (padding ou marging) como modificar de estilo.

<br />

## Quando não usar:
- Nos casos em que determinado componente precisar de vários modificadores de estilo. Pra esses casos
utilize classes e nelas inclua paddings e margins com os tokens de espaçamento do Cuida.

<br />

## Observações:
- ⚠️ Dê preferência sempre que possível à utilização dos tokens em detrimento ao Spacer.
- O valor mandado para as props do Spacer é sempre multiplicado por 4, assim como nos tokens scss do Cuida.
- O valor máximo aceito é 12.
- O spacer não implementa margins e paddins negativos.

## Preview

<Canvas>
	<Story
		name="Spacer"
		args={{
			paddingTop: 0,
			paddingRight: 0,
			paddingBottom: 0,
			paddingLeft: 0,
			marginTop: 0,
			marginRight: 0,
			marginBottom: 0,
			marginLeft: 0,
		}}
	>
		{ Template.bind({}) }
	</Story>
</Canvas>

<ArgsTable story="Spacer" />
