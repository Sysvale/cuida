...
- `pTRLB()` e `mTRLB()` -> Funções que permitem que sejam especificados espaçamentos distintos para 
...
- `spacer()` -> Função que permite a utilização da escala de espaçamento do Cuida com propriedades de margin
 e paddin mais específicas, como, por exemplo: `margin-top`, `padding-left`, etc.