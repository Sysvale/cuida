import { Meta } from '@storybook/addon-docs/blocks';

<Meta title="Fundação/Princípios" />

# Bem-vindo ao Cuida

#### O cuida é o design system do Cidade Saudável. É a materialização de todos os nossos princípios e fundação, englobando a nossa maneira de pensar design, implementação e o cuidado com as pessoas.

<br />

## Princípios:
<br />

### Contexto acima da consistência
##### Consistência e padrões são importantes para a construção de uma linguagem unificada de design que ajuda o usuário a saber o que esperar do nosso produto e como utilizá-lo. Entretanto, isso não deve significar sacrificar a experiência de uso. Nesse sentido, o contexto e as necessidades dos nossos usuários são prioridades ao desenvolvermos nossas soluções.
<br />

### Usável
##### Usuários e suas necessidades orientam nosso processo de design. Para nós, ser usável é ser construído com empatia, alcançando as reais necessidades e problemas dos usuários ao nos colocarmos no seu lugar e olhar para seu mundo com atenção e cuidado. Para atingirmos isso, realizamos pesquisa, analisamos dados e os ouvimos sempre.
<br />

### Inclusivo
##### Projetamos nossas experiências para pessoas reais. Entendemos que  aqueles que utilizam nossos produtos têm diferentes condições e habilidades. Essa compreensão nos guia no desenvolvimento de soluções mais humanas.
<br />

### Direto
##### Todos os elementos devem ter um propósito. Esses elementos devem fornecer experiências que permitam às pessoas atingir seus objetivos de maneira rápida, eficiente e o mais direta possível.
<br />

### Autonomia pelo empoderamento
##### Acreditamos no potencial das pessoas e na sua capacidade de desenvolver as melhores soluções. Desse modo, empoderamos os times para que se sintam seguros ao projetarem soluções, sabendo que o farão garantindo a melhor experiência de uso enquanto trabalham com o que são apaixonados em fazer.

